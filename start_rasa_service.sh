...
# 如果需要指定actions服务端口(默认5055)，通过--port指定，同时endpoints.yml中也需要进行更改
# nohup rasa run actions --port 5056 &> log/rasa_actions.log &
...
## 解决客户端和rasa服务器跨域问题
# 如果需要指定rasa服务端口(默认5005)，通过--port指定
# nohup rasa run --cors "*" --enable-api --port 5006 &> log/rasa_server.log &
