...
from .models.vectorizers import BERTVectorizer
from .models.vectorizers import TagsVectorizer
from .models.dataset import NluDataset
from .models.dataset import flatten, convert_to_slots
...
class JointNLU(object):
    ...
    def predict(self, utterance: str):
        ...
        predicted_tags, predicted_intents = self.model.predict_slots_intent(x, self.tags_vectorizer,
                                                                            self.intents_label_encoder,
                                                                            remove_start_end=True,
                                                                            include_intent_prob=True)
        slots = convert_to_slots(predicted_tags[0])
        slots = [{"slot": slot, "start": start, "end": end, "value": ' '.join(tokens[start:end + 1])} for
                 slot, start, end in slots]
        response = {
            "intent": {
